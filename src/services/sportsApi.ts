...
export async function fetchLiveFixtures(): Promise<any[]> {
  if (!isSportsApiConfigured) {
    console.warn(
      'API-Football no está configurada en .env (VITE_SPORTS_API_KEY). Usando simulador local.',
    );
    return [];
  }
...
export async function fetchFixtureStatistics(
  ...
  try {
    ...
    const getStat = (teamIndex: number, type: string): number => {
      const stat = teamsStats[teamIndex]?.statistics?.find(
        (s: any) => s.type === type,
      );
    ...
    console.error(
      `Error al obtener estadísticas del partido ${fixtureId}:`,
      error,
    );
    return null;
  }
