...
export const AnalyticsModal: React.FC<AnalyticsModalProps> = ({ isOpen, onClose }) => {
  ...
  const sportBreakdown = computedBreakdown.length > 0
    ...
    : [
        { sport: '⚽ Fútbol (Champions & Ligas)', bets: 0, winRate: 0, profit: 0 }
      ]
        ...
        <div className="bg-gradient-to-r from-[#161822] via-[#241A12] to-[#161822] border border-orange-500/30 rounded p-4 mb-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.2 bg-emerald-950/80 text-emerald-400 border border-emerald-500/30 text-[10px] font-mono rounded">
                  TOP 5%
                </span>
              </div>
              <p className="text-xs text-slate-400 font-mono-numbers">
                Rating actual: <strong className="text-orange-400">{stats.eloRating} ELO</strong> | Próximo rango en: {200 - (stats.eloRating % 200)} pts
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 font-mono">
            <div className="px-3 py-1.5 bg-[#0B0C10] rounded border border-white/10 text-center">
              <span className="text-[10px] text-slate-400 uppercase block">Racha Actual</span>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {sportBreakdown.map((sb, i) => (
              <div key={i} className="p-3 bg-[#0B0C10] rounded border border-white/5 flex items-center justify-between gap-2 text-xs">
                <div>
                  <span className="font-semibold text-white block">{sb.sport}</span>
                  <span className="text-slate-400 text-[11px] font-mono-numbers">{sb.bets} apuestas registradas</span>
                </div>
