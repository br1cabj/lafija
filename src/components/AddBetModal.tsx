...
export const AddBetModal: React.FC<AddBetModalProps> = ({
  ...
  const [betType, setBetType] = useState<'single' | 'parlay' | 'bet_builder'>(
    'bet_builder',
  );
          ...
          <div className='flex items-center gap-2.5'>
            ...
            <div>
              ...
              <p className='text-xs text-slate-400'>
                Configura los mercados, cuotas en cualquier formato y
                condiciones
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div className='space-y-2.5'>
            ...
            {conditions.map((c, idx) => (
              <div
                key={idx}
                className='p-3 bg-[#0B0C10] rounded border border-white/10 flex flex-col sm:flex-row gap-2.5 items-start sm:items-center'
              >
                <input
                  type='text'
