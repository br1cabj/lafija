...
export async function generateSlipBlob(
  ...
  const height = headerHeight + conditionsBoxHeight + financialsHeight + footerHeight + 36
  ...
  ctx.fillText(`${metCount}/${conditions.length} CUMPLIDAS`, width - 38, condY + 24)
  ...
  conditions.forEach((cond, i) => {
    ...
    // Row Background Pill
    ctx.fillStyle = isMet ? 'rgba(16, 185, 129, 0.14)' : 'rgba(255, 255, 255, 0.04)'
    drawRoundedRect(ctx, 36, rowY, width - 72, 36, 8)
    ctx.fill()

    ctx.strokeStyle = isMet ? 'rgba(16, 185, 129, 0.4)' : 'rgba(255, 255, 255, 0.06)'
    ctx.lineWidth = 1
    drawRoundedRect(ctx, 36, rowY, width - 72, 36, 8)
    ctx.stroke()
    ...
    ctx.font = 'bold 13px system-ui, -apple-system, sans-serif'
    const label = cond.selection.length > 36 ? cond.selection.slice(0, 34) + '...' : cond.selection
    ...
    const valText = typeof cond.currentValue === 'number' && typeof cond.targetValue === 'number'
  ...
  ctx.fillStyle = '#64748B'
  ctx.font = 'bold 11px system-ui, -apple-system, sans-serif'
  ctx.fillText(`CUOTA (${oddsFormat.toUpperCase()})`, 24 + colW * 1.5, finY + 30)
  ctx.fillStyle = '#FF5500'
